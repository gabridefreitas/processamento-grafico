...
// Essencial para computadores da Apple
  ...
  GLFWwindow *window = glfwCreateWindow(
      WIDTH, HEIGHT, "Hello Triangle! - Gabriel", nullptr, nullptr);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glUniform4f(colorLoc, 0.0f, 1.0f, 0.0f,
                1.0f);  // enviando cor para variável uniform inputColor

    // glDrawArrays(GL_LINE_LOOP, 0, 3);
    glDrawArrays(GL_LINE_LOOP, 3, 3);

    glUniform4f(colorLoc, 1.0f, 1.0f, 0.0f,
                1.0f);  // enviando cor para variável uniform inputColor
...
// programa de shader simples e único neste exemplo de código O código fonte do
// vertex e fragment shader está nos arrays vertexShaderSource e fragmentShader
// source no início deste arquivo A função retorna o identificador do programa
// de shader
int setupShader() {
  // Vertex shader
...
// Esta função está bastante hardcoded - objetivo é criar os buffers que
...
int setupGeometry() {
  ...
  // normal, etc) Pode ser armazenado em um VBO único ou em VBOs separados
  GLfloat vertices[] = {
  ...
  // Para cada atributo do vértice, criamos um "AttribPointer" (ponteiro para o
  // atributo), indicando:
