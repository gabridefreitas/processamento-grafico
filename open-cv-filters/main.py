...
def handleKeyboardInput(saveIndex, inputKey, originalImage, visibleImage):
    ...
    if inputKey == ord("1"):
        ...
        filteredColor = originalImage.copy()

        for i in range(filteredColor.shape[0]):  # percorre linhas
            for j in range(filteredColor.shape[1]):  # percorre colunas
                media = (
                    filteredColor.item(i, j, 0) * 0.333
                    + filteredColor.item(i, j, 1) * 0.333
                    + filteredColor.item(i, j, 2) * 0.3333
                )
                filteredColor.itemset((i, j, 0), media)  # canal B
                filteredColor.itemset((i, j, 1), media)  # canal G
                filteredColor.itemset((i, j, 2), media)  # canal R

        result[2] = filteredColor
    elif inputKey == ord("s"):
